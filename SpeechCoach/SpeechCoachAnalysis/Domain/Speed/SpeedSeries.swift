...
public struct SpeedSeries: Codable, Hashable {
    public let binSeconds: TimeInterval
    public let bins: [SpeedBin]
    ...
    public var maxWPM: Double { bins.map(\.wpm).max() ?? 0 }
    public var minWPM: Double { bins.map(\.wpm).min() ?? 0 }
